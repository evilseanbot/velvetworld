...
namespace Obi
{
	...
	public abstract class ObiRopeBase : ObiActor
	{
		...
		public class CurveFrame{
			...
			public void Transport(CurveSection section, float twist){
			...
			public void Set(CurveSection section){
		...
		public struct CurveSection{
			...
			public CurveSection(Vector4 positionAndRadius, Vector3 tangent, Vector3 normal, Vector4 color){
		    ...
		    {
				return new CurveSection(c1.positionAndRadius + c2.positionAndRadius,c1.tangent + c2.tangent,c1.normal + c2.normal,c1.color + c2.color);
		    }

			public static CurveSection operator *(float f,CurveSection c) 
		    {
				return new CurveSection(c.positionAndRadius * f, c.tangent * f, c.normal * f, c.color * f);
		    }
		}
			...
			for (int i = 0; i < rawCurves.Count; ++i){
				...
				for (int m = 0; m < segments; ++m){
					...
					if (GetStructuralConstraintParticles(firstSegment + m,ref particle1,ref particle2)){
						...
						controlPoints[m] = new CurveSection(new Vector4(pos.x,pos.y,pos.z,principalRadii[particle1][0]),tangent,orient * Vector3.up,color);
				...
				if (segments > 0){
					...
					controlPoints[segments] = new CurveSection(new Vector4(pos.x,pos.y,pos.z,principalRadii[particle2][0]),tangent,orient * Vector3.up,color);
				}
